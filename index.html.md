...
- If that doesn’t work one can also install quarto following other
  instructions
...
```
    ...
    remove_label(path: str | pathlib.Path, *, mode: popidd_sveil.core.OperationMode | str = <OperationMode.COPY: 'copy'>, output: str | pathlib.Path | None = None, overwrite: bool = False, relative_tolerance: float = 0.01) -> popidd_sveil.core.AnonymizationResult
...
``` python
source_info = inspect_slide(image_path)
...
with TemporaryDirectory() as directory:
    output = Path(directory) / image_path.name
